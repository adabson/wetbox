...
PERMIT_N_CLASHING = 4; //limit 4 clashes per ticket AND 4 clashes per square(3pair)
...
for(let i=0;i<NcK;i++) { // < NcK
  ...
  for(let j=0;j<20;j++) {
    ...
    if(clash>PERMIT_N_CLASHING || state[pairI]>=PERMIT_N_CLASHING) {
      break;
    }
  ...
  if(tikn_20_3_pairs.length===20) { //we made it the whole way without experiencing a clash)
    ...
    console.log('Evergreen: ',tiks.length,'('+coverage+'%)')
